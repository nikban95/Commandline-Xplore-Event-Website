...
#             back="/command_line/"+(str(offset-1))+("/")
#             next="/command_line/"+(str(offset+1))+("/")
#             submit="/command_line/"+(str(offset))+("/")
#             temp=get_template('ques.html')
#             returnobj=temp.render(RequestContext(request,{'next':next,'submit':submit,'ques1':ques1,'prevans':prevans,'back':back,
#             'done':"/done/",'flag':flag,'back_value':back_value,'next_value':next_value,'offset':offset}))
#             return HttpResponse(returnobj)
#         else:
#             return  render_to_response('error_message.html')    
    #else:
     #   c=RequestContext(request,{'submit':"/command_line/registered/"})
